...
function getCachedDashboardKey(host) {
  const cache = loadCache();
  return cache.dashboard_keys?.[host] ?? null;
}
function cacheDashboardKey(host, key) {
  const cache = loadCache();
        ...
        for (const method of methods) {
          tools.push({
            ...
            inputSchema: {
              ...
              required: method.params.filter((p) => p.default === null).map((p) => p.name)
            },
            capName: cap.name,
...
async function fetchDashboardApiKey(cogentName, host) {
  ...
  try {
    ...
    const raw = (0, import_child_process.execSync)(cmd, { timeout: 1e4, encoding: "utf-8" }).trim();
...
async function connectRemoteMcp() {
  ...
  remoteTransport = new import_streamableHttp.StreamableHTTPClientTransport(mcpUrl, {
    requestInit: { headers: headers() }
  });
  remoteClient = new import_client.Client(
...
async function refreshRemoteTools() {
...
async function disconnectRemoteMcp() {
  if (remoteTransport) {
    try {
...
async function disconnect() {
  ...
  return wasConnected ? `Disconnected from cogent "${oldCogent}". Use connect to connect to another cogent.` : "Not connected.";
}
...
var mcpServer = new import_server.Server(
  { name: "cogos", version: "2.0.0" },
  {
    ...
    instructions: "CogOS plugin. Use the `connect` tool to connect to a cogent (e.g. connect with address 'alpha.softmax-cogents.com'). Use 'process@address' to connect to a specific process (default: supervisor). After connecting, use `load_memory` to get the process's full instructions."
  }
);
var CONNECT_TOOL = {
  name: "connect",
  description: "Connect to a CogOS cogent process. Use 'process@address' to connect to a specific process (default: supervisor). Examples: 'alpha.softmax-cogents.com', 'supervisor@alpha.softmax-cogents.com', 'discord@alpha'. If no token is cached, creates one automatically via the API.",
  inputSchema: {
    type: "object",
    properties: {
      address: {
        type: "string",
        description: "Cogent address with optional process prefix (e.g. 'alpha.softmax-cogents.com', 'supervisor@alpha.softmax-cogents.com')"
      },
      token: {
        type: "string",
...
var DISCONNECT_TOOL = {
  name: "disconnect",
  description: "Disconnect from the current cogent. Use this to switch to a different cogent without restarting Claude Code.",
  inputSchema: {
    type: "object",
    properties: {}
  }
};
var REGISTER_EXECUTOR_TOOL = {
  name: "register_executor",
  description: "Register as an executor for the connected cogent. Enables heartbeat, channel notifications, and run assignments.",
  inputSchema: {
    type: "object",
    properties: {}
  }
};
var COMPLETE_RUN_TOOL = {
  name: "complete_run",
...
mcpServer.setRequestHandler(import_types.ListToolsRequestSchema, async () => {
  const tools = [];
...
mcpServer.setRequestHandler(import_types.CallToolRequestSchema, async (req) => {
