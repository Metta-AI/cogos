...
def get_status(name: str, range: str = Query("1h", alias="range")):
    ...
    row = repo.query_one(
        ...
        "(SELECT count(*) FROM triggers WHERE enabled = true) AS trigger_count, "
        "(SELECT count(*) FROM alerts WHERE resolved_at IS NULL) AS unresolved_alerts, "
        f"(SELECT count(*) FROM events WHERE created_at > now() - interval '{interval}') AS recent_events",
    )
    return StatusResponse(cogent_name=name, **(row or {}))
