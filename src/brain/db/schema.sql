...
CREATE UNIQUE INDEX IF NOT EXISTS idx_memory_unique_name ON memory (cogent_id, scope, name) WHERE name IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_memory_scope ON memory (cogent_id, scope, type);
CREATE INDEX IF NOT EXISTS idx_memory_name ON memory (cogent_id, name) WHERE name IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS channels (
    ...
    enabled     BOOLEAN NOT NULL DEFAULT true,
    created_at  TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE (cogent_id, type, name)
);
CREATE INDEX IF NOT EXISTS idx_channels_type ON channels (cogent_id, type);
...
CREATE INDEX IF NOT EXISTS idx_conversations_context ON conversations (cogent_id, context_key) WHERE status != 'closed';
...
CREATE INDEX IF NOT EXISTS idx_alerts_unresolved ON alerts (cogent_id, created_at DESC) WHERE resolved_at IS NULL;
...
CREATE TABLE IF NOT EXISTS budget (
    ...
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE (cogent_id, period, period_start)
);
