...
export async function getProcessDetail(
  name: string,
  processId: string,
): Promise<{ process: CogosProcess; runs: CogosProcessRun[]; resolved_prompt: string; file_keys: string[]; capabilities: string[]; capability_configs: Record<string, Record<string, unknown>>; includes: Array<{ key: string; content: string }> }> {
